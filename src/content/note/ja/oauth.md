...
series: Comment
tags: [Guide, OAuth]
description: GitHub、Google などの複数プロバイダーに対応した OAuth 2.0 認証の詳細設定ガイド。
---
...
プロバイダー設定では、`Website URL` にサイト URL を、`Callback URI` に `<サイト URL>/@/reach/<プロバイダー>` を記入してください。

## OAuth プロバイダーの拡張

新しい OAuth プロバイダーを追加する必要がある場合は、以下の手順または [Arctic ドキュメント](https://arcticjs.dev/)を参照し、`src/lib/oauth.ts` で対応するロジックを補完してください。
