...
1. 用户提交数据
    ...
    - 电子邮箱地址（OAuth 登录用户可选）：用户个人的电子邮箱地址。
...
| 电子邮箱地址 | 用户输入 | 接收回复通知 | Cloudflare D1 数据库 | OAuth 登录用户可选 |
...
- localStorage：用户的自定义昵称、主题偏好设置和评论草稿存储在浏览器的 localStorage 中，这些数据仅在用户本地设备上存储，不会传输到服务器。用户可以随时通过浏览器设置清除这些本地数据。
