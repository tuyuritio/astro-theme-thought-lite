...
<figure class="fixed top-0 left-0 w-full h-full flex flex-col pr-5 z-3 pointer-events-none overflow-hidden">
	{#each $tips as tip (tip)}
		<section animate:flip={{ duration: 200, easing: circInOut }} transition:fly={{ y: -100, opacity: 0, duration: 200, easing: circInOut }} class="relative flex items-center gap-2 ml-auto mt-7 border-2 border-solid border-weak rounded-sm py-4 px-3 w-xs bg-background shadow-md pointer-events-auto">
