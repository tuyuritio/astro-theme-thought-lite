...
<main id={comment.id} class:before:hidden={depth >= MIN_DEPTH} class:sm:before:block={depth < Math.max(MIN_DEPTH, MAX_DEPTH)} class="relative before:absolute before:content-[''] before:top-12 before:start-4 before:h-[calc(100%-2rem)] before:w-[0.2rem] before:bg-[linear-gradient(var(--shadow-color)_calc(100%-2rem),transparent)]">
	...
	<div class:ms-7={depth < MIN_DEPTH} class:sm:ms-7={depth < Math.max(MIN_DEPTH, MAX_DEPTH)}>
		{#if replyView && !editView}
			<Reply {locale} {link} {oauth} {turnstile} {drifter} bind:notification section={comment.section} item={comment.item} reply={comment.id} {refresh} bind:view={replyView} bind:limit />
		{:else if editView && !replyView}
			<Reply {locale} {link} {oauth} {turnstile} {drifter} bind:notification section={comment.section} item={comment.item} reply={comment.reply} edit={comment.id} text={comment.content} {refresh} bind:view={editView} bind:limit />
		{/if}

		{#each comment.subcomments as subcomment}
			<Self {locale} {link} {oauth} {turnstile} {drifter} bind:notification comment={subcomment} {refresh} depth={depth + 1} bind:limit />
		{/each}
	</div>
</main>
