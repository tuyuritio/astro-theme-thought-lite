{#if open}
	<dialog bind:this={dialog} onmousedown={event => (event.target == dialog ? (open = false) : null)} transition:fade={{ duration: 150 }} class="fixed m-a border-2 border-solid rounded-md c-primary bg-background backdrop:bg-transparent shadow-md">
